...
export function adminAionHandler(req, res) {
                    ...
                    ConnectAionGS.connect((error) => {
                        ...
                        ConnectAionGS.query(query3, (error, results3) => {
                            ...
                            const connections = [
                                ...
                                { server: 'localhost', port: 7777, timeout: 5000 },
                                { server: 'localhost', port: 2106, timeout: 5000 },
                                { server: 'localhost', port: 11000, timeout: 5000 },
                            ...
                            Promise.all(connectionPromises)
                                .then((results) => {
                                    ...
                                    const puerto7777 = results.find((result) => result.port === 7777 && result.success)
                                        ? '7777'
                                        : 'error 7777';

                                    const puerto2106 = results.find((result) => result.port === 2106 && result.success)
                                        ? '2106'
                                        : 'error 2106';

                                    const puerto11000 = results.find((result) => result.port === 11000 && result.success)
                                        ? '11000'
                                        : 'error 11000';

                                    res.render('admin-aion', {
                                        feudalinfo: feudalAccountCount,
                                        datos2: results2,
                                        datos3: results3,
                                        puerto3306: puerto3306,
                                        puerto7777: puerto7777,
                                        puerto2106: puerto2106,
                                        puerto11000: puerto11000
                                    });
                                })
                                .catch((error) => {
...
export default adminAionHandler;